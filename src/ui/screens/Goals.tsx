...
export const Goals = ({ refreshKey, onAdd, onEdit, onChanged, notify }: {
  ...
  const remove = async (item: Goal) => {
    if (!window.confirm(`Excluir o objetivo “${item.name}”?`)) return;
    await window.lionPocket.deleteGoal(item.id);
        ...
        {items.map((goal) => (
          <article className="goal-card" key={goal.id}>
            ...
            <footer>{goal.link ? <button className="text-button" onClick={() => window.lionPocket.openExternal(goal.link)}>Abrir link <ExternalLink size={14} /></button> : <span />}
              <div><button className="icon-button" onClick={() => onEdit(goal)} title="Editar"><Pencil size={16} /></button><button className="icon-button icon-button--danger" onClick={() => remove(goal)} title="Excluir"><Trash2 size={16} /></button></div>
            </footer>
          </article>
        ))}
      </div>
